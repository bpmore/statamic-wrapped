...
- [x] Upload: a "Your soundtracks" assets field on the settings screen (same container as the logo, filled in at boot, `allow_uploads`, several files). Only files with an audio extension (mp3, m4a, wav, ogg, flac, aac) become tracks; anything else is ignored, and the instructions say MP3 is the safe choice. An uploaded track joins `Soundtracks` between the site's config tracks and the bundled ones, handle `upload-{asset id}`, name from the asset's title or filename, so it appears in the video maker and the control panel story with a preview like the others
...
### Phase 9, task 1 — Upload (done)
- Settings screen has a Music section with a "Your soundtracks" assets field (`mode: list`, `allow_uploads`, `show_filename`, no `max_files`), given the site's first container at boot like the logo. A site with no container gets neither the field nor an empty Music section (tested).
- `SavedSettings` now holds what `ThemeSettings` and `Soundtracks` both need: the raw saved record, and an assets-field value back to an `Asset` (path in any container, or `container::path`). `ThemeSettings` delegates to it; its behaviour is unchanged and its tests still pass.
- `Soundtracks::uploaded()`: each saved value → asset → `isAudio()` (Statamic's own list: aac, flac, m4a, mp3, ogg, wav) → `resolvedPath()` must be a file, else logged and skipped (task 2 will copy non-local files). Handle `upload-` + 12 hex of sha1(asset id), stable and URL-safe. Name from the asset title, else the filename humanised ("house-theme_v2" → "House Theme V2"). Description "Uploaded to this site." Order: config, uploads, bundled; an upload becomes the default when there are no config tracks.
- Nine new tests in `SettingsScreenTest`; removed the audio filter to confirm the "not audio" test fails, then restored it. (Restoring by `git checkout` threw away the whole uncommitted file once; redone from the same edit script. Lesson noted below.)
- Verified on statamic-dev: uploaded a 20-second MP3 through the settings screen, saved (`resources/addons/wrapped.yaml` lists it, file in `public/assets`), it appeared first in the video maker's picker as "House Theme, Uploaded to this site.", Preview streamed it (HTTP 206, full bytes), and a video rendered with it has an AAC audio stream at mean −18.9 dB, so the upload is really in the MP4.
- Working note: never `git checkout <file>` to undo a one-line experiment on a file with uncommitted work; reverse the one line instead, or stage first.

### Phase 9, task 2 — Disk (done)
- `Soundtrack::path` is now nullable, with an optional `Asset`. `localPath()` returns the file, or copies the asset's stream to `sys_get_temp_dir()/wrapped-track-{handle}-{rand}.{ext}` once and returns that; `release()` deletes the copy. `stream()` gives the bytes for the preview route either way; `isLocal()`, `extension()`; `mimeType()` reads the extension from whichever it has.
- `Soundtracks::uploaded()` decides local by `$asset->disk()->filesystem()->getAdapter() instanceof LocalFilesystemAdapter`. Local: `resolvedPath()`, must be a file. Not local: no path, the asset is carried. (`resolvedPath()` on a non-local disk throws a LogicException in Statamic's adapter, so the check has to come first.)
- `WrappedVideo::render()` passes `localPath()` inside try/finally with `release()`, so the copy lasts exactly one render, failure included. The preview route sends a file as before, or streams the asset through.
- Tests: `NotLocalAdapter` fixture wraps a real local Flysystem adapter so it behaves but is not `LocalFilesystemAdapter`; a `notlocal` Storage driver in the test. Six tests in `RemoteSoundtrackTest`: no path, copy/same copy/release, file exists during render and not after, cleaned up on a failed render, preview streams the bytes, missing file left out. Commented out `release()` and both cleanup tests failed; restored.
- Not exercised against real S3; the adapter check and the stream copy are the only S3-specific parts and both go through Flysystem's interface.

### Phase 9, task 3 — Music on the public page is a choice (done)
- Share form: a "Music" group with None (default), A track (a select of every track the video can have, bundled/config/uploaded, with a Preview button that streams from the CP soundtrack route), or A YouTube video (the link field, shown only when chosen). One line under it: either kind plays on the shared web page only. Each live link shows "Music: {name}" for a track as it does for a video.
- `wrapped_shares.soundtrack` (string 64, nullable), its own migration since 1.5 shipped. `Share::soundtrack(Soundtracks)` resolves the handle or null when the track is no longer offered. `ShareLinks::publish()` refuses a track and a video together and an unknown handle; the controller validates `soundtrack` with `Rule::in` of the current handles and turns "both" into a validation error on `music` before asking YouTube anything.
- Public route `wrapped/{token}/music` → `PublicStoryController::music()`: same resolve as the page (live link or 404), then the track or 404, served by `SoundtrackResponse` (file with ranges, or streamed through from a remote container). The CP preview route uses the same class now.
- Page: one set of buttons for either kind (`$hasMusic`). A track is a looping `Audio()` made on the first press with `preload="none"`, so nothing is fetched before that; if `play()` rejects the buttons go back to "Play music". No tile, no `.has-music`, and not one mention of YouTube on a track page (tested). A track that has gone (upload deleted, bundled switched off) means no buttons and a 404 from the music route; the row keeps its handle.
- Eight new tests. Two first-run surprises were the tests catching literal words in comments (`<audio`, `YouTube`) that the page must not carry; reworded the comments rather than the tests.
- Verified on statamic-dev: made a Q3 link with the uploaded "House Theme", logged out, pressed Play music: the page fetched `/wrapped/{token}/music` (206, all 382,988 bytes), both buttons read "Stop music" and stayed so, no tile, stage unchanged.

### Phase 9, task 4 — Wording and docs (done)
- README "Music": the control panel upload comes first, config second; a "rights your own music needs" paragraph (made by you, made for you, or library music licensed for video and web use: yes; a bought or streamed song: no; the bundled tracks: covered for both; the question to ask a library). "Music on a public link" replaces "A song on a public link": the three-way choice, how a track streams and stops with the link, then the YouTube part as before. "What it will not do" adds that a track on a public page is one the site chose, streamed by the site.
...
- Upgrade on `~/Herd/wrapped-fresh`, 1.5.1 → 1.6.0 via `composer update --prefer-dist` from the GitHub dist zip: Statamic republished the `wrapped` assets during the update (the published manifest hash matches the package's), `php artisan migrate` ran the one new migration, and the 1.5.1 share link came through untouched (same token, voice `we`, its YouTube id, five cards, `soundtrack` null). Its page still answers 200; its music route answers 404, as a link with no track should.

### After 1.6.0 — marketplace review, 1.6.1 (done)
- Second `/marketplace-review`, against v1.6.0: nothing required; one optional tightening taken up. `SavedSettings::asset()` searched `AssetContainer::all()` in whatever order it came, while the form always offered the first container by handle; on a site with two containers holding the same relative path the wrong file could be read back (logo since 1.3, soundtracks since 1.6). Now `SavedSettings::containers()` is one sorted ordering used by both `formContainer()` (the provider draws the form from it) and the read-back, so they cannot disagree. Test with containers `aaa`, `uploads` and `zzz` all holding `music/jingle.m4a`: the track is aaa's, and `zzz::music/jingle.m4a` is taken at its word. Reversing the ordering made the test fail; restored.
- PHPStan detail: Statamic's `AssetContainer` contract documents `asset()` as never null while the class documents (truthfully) `Asset|null`, so the code types containers by the class, not the contract, rather than silencing the analyser.
- Slip: tried `git stash -- <file>` to run the old code for comparison; it stashed the one file and left the provider referring to a method that no longer existed. Popped at once, nothing lost; the comparison was done with a reversible one-line sed instead. Same lesson as the `git checkout` note above: on a file with uncommitted work, reverse the line, not the file.
- Upgrade on `~/Herd/wrapped-fresh`, 1.6.0 → 1.6.1 via `composer update --prefer-dist` from the GitHub dist zip: nothing to migrate (as the changelog says), published asset hash unchanged and equal to the package's, no dev files in the archive. The 1.5.1-era share link came through unchanged again (same token, voice `we`, YouTube id, five cards; page 200). Exercised the fixed path: with the site's one container `assets`, `SavedSettings::formContainer()` and the blueprint's `soundtracks` field agree on `assets`; an M4A put in it and saved on the field came back as track `upload-98ea83d89e51` "Upgrade Check", local, 3,580,079 bytes.

### Marketplace submission (20 September 2026)
- Repo made public (secrets and local paths scanned first; anonymous fetch of the page and the v1.6.1 zip both 200). Packagist submission by Brent. Statamic listing submitted 20 September 2026: categories Analytics, Social Media, Widget; minimum version 1.6.1; free; support URL the GitHub issues page; eight screenshots in the order in `docs/marketplace.md`; description assembled from `docs/marketplace.md`.
- The listed 1.6.1 still says "colour" in the settings instructions and the control panel; the American-spelling sweep landed on main after the tag. A 1.6.2 tag would bring the installed package in line with the README the marketplace shows.
